...
ARG MOTOR_VERSION=R7-2-2
ARG MOTOR_URL=https://github.com/epics-modules/motor.git
ENV MOTOR ${EPICS_MODULES}/motor-${MOTOR_VERSION}
RUN set -x;\
    set -e;\
    cd ${EPICS_MODULES};\
    git clone ${MOTOR_URL} --branch ${MOTOR_VERSION} ${MOTOR} ;\
    cd ${MOTOR} ;\
    git submodule init ;\
    git submodule update modules/motorMotorSim ;\
    sed -i \
        -e '12cEPICS_BASE='${EPICS_MODULES} \
        -e '17cASYN='${ASYN} \
        -e '21cSNCSEQ='${SNCSEQ} \
        -e '23cBUSY='${BUSY} \
        -e '26cEPICS_BASE='${EPICS_BASE} \
        -e '43cMOTOR=$(TOP)' \
        configure/RELEASE ;\
    sed -i \
        -e '39cBUILD_IOCS=YES' \
...
RUN cd ${MOTOR} ;\
    ...
    sed -i \ 
        -e '1c#!../../bin/'${EPICS_HOST_ARCH}'/motorSim' \
        st.cmd.unix ;\
    chmod +x st.cmd.unix

COPY ["motor.substitutions", "init.sh", "${MOTOR}/modules/motorMotorSim/iocs/motorSimIOC/iocBoot/iocMotorSim/"]
WORKDIR ${MOTOR}/modules/motorMotorSim/iocs/motorSimIOC/iocBoot/iocMotorSim
